...
from .utils import calculate_mpf, predict_image, crop_center_square, label_to_pixelvalue
...
parser.add_argument(
    "--data",
    default="data/prediction/raw/220718_2/IRdata_ATWAICE_processed_220717_122355.nc",
    type=str,
...
parser.add_argument(
    "--weights_path",
    default="experiments/AutoSamPonds3/model.pth",
    type=str,
...
parser.add_argument(
    "--metrics_path",
    default="metrics/melt_pond_fraction/",
    type=str,
    help="Path to .csv file that should store the resulting mpf (if calculation is desired).",
)
parser.add_argument(
    "--model_type",
    default="vit_b",
...
def main():
    ...
    print("Model architecture used: ".format())
    ...
    if not params["skip_preprocessing"]:
        ...
        if "/" in params["data"]:
            ...
            ds = netCDF4.Dataset(
                os.path.join("data/prediction/raw", id, params["data"])
            )
        ...
        for idx, img in enumerate(imgs):
            plt.imsave(
                os.path.join(params["preprocessed_path"], "{}.png".format(idx)),
                img,
                cmap="gray",
            )
                ...
                masks.append(
                    predict_image(
                        ...
                        save_path=os.path.join(
                            params["predicted_path"], "raw/{}.png".format(id)
                        ),
                        visualize=False,
                        normalize=params["normalize"],
                        no_finetune=params["no_finetune"],
                        model_arch=params["model"],
                    )
                )
    ...
    if params["convert_to_grayscale"]:
        ...
        for idx, file in enumerate(
            os.listdir(os.path.join(params["predicted_path"], "raw/"))
        ):
            id = file.split(".")[0]
            im = label_to_pixelvalue(
                cv2.imread(os.path.join(params["predicted_path"], "raw/", file))
            )
            cv2.imwrite(
                os.path.join(params["predicted_path"], "grayscale/{}.png".format(id)),
                im,
            )
    ...
    if not params["skip_mpf"]:
        ...
        with open(
            os.path.join(params["metrics_path"], "mpf.csv"), "a", newline=""
        ) as f:
            writer = csv.writer(f)
