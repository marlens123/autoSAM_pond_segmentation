...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.save_dir = "experiment/" + args.save_dir
    print(args.save_dir)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if loss < best_loss:
            is_best = True
            best_loss = loss
            torch.save(model.state_dict(), args.save_dir + '/feat_seg_model.pth')
...
def train(train_loader, class_weights, model, optimizer, scheduler, epoch, args, writer):
    ...
    dice_loss = SoftDiceLoss(batch_dice=True, do_bg=False, rebalance_weights=class_weights)
