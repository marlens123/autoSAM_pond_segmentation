    ...
    for i in range(0, image.shape[0], patch_size):
        for j in range(0, image.shape[1], patch_size):
            ...
            single_patch = preprocess_prediction(
                single_patch, model_preprocessing=model_preprocessing
            )
            ...
            segm_img[i : i + single_patch_shape[0], j : j + single_patch_shape[1]] += (
                cv2.resize(fin, single_patch_shape[::-1])
            )
...
def single_patch_predict(
    ...
    image = preprocess_prediction(
        image, model_preprocessing=model_preprocessing, normalize=normalize
    )
...
def predict_image(
    ...
    if smooth:
        ...
        segmented_image = single_patch_predict(
            model,
            img,
            model_preprocessing=prepro,
            visualize=visualize,
            normalize=normalize,
